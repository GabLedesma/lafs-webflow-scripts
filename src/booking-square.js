  ...
  document.body.addEventListener("click", async (event) => {
    ...
    try {
      ...
      payBtn.onclick = async (event) => {
        ...
        try {
          const response = await fetch("https://createsquarepaymentlink-xmismu3jga-uc.a.run.app", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              slug,
              purchaseData: {
                ...
                promoValue: promoState?.value || 0,
                discountPerUnit: promoState?.discountPerUnit || 0,
